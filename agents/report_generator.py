...
def _build_report(f: dict) -> str:
    ...
    if nama_list:
        rows = []
        for i, nm in enumerate(nama_list):
            jw = jawatan_list[i] if i < len(jawatan_list) else ""
...
def handle(query: str, history: list[dict] | None = None, session_id: str = "default", lang: str = "bm", user_name: str = "") -> str:
    ...
    if query == '__INTRO__':
        if lang == "en":
            return (f"Assalamualaikum and welcome{sapaan}! 📋 I am the Report Generator. I am ready to help you generate a one-page report (One Page Report) in the official PPD/KPM format. Could you tell me the name of the programme or activity to be reported?\n\n"
        ...
        return (f"Assalamualaikum dan selamat datang{sapaan}! 📋 Saya Penjana Laporan. Saya sedia membantu anda menjana laporan satu muka surat (One Page Report) dalam format rasmi PPD/KPM. Boleh beritahu saya nama program atau aktiviti yang ingin dilaporkan?\n\n"
    ...
    # ── Guard: reject letter requests — redirect to Letter Writer agent ──
    _LETTER_KEYWORDS_BM = {
        "jana surat", "tulis surat", "buat surat", "hasilkan surat",
        "surat rasmi", "surat jemputan", "surat peringatan", "surat makluman",
        "surat tawaran", "surat lantikan", "surat arahan", "write letter",
        "generate letter", "create letter", "draft letter",
    }
    query_lower_check = query.lower()
    if any(kw in query_lower_check for kw in _LETTER_KEYWORDS_BM):
        if lang == "en":
            return (
                "I'm the **Report Writer** — I can only generate programme reports, meeting minutes, "
                "and inspection reports. I'm unable to write official letters.\n\n"
                "👉 To write an official letter, please switch to the **✍️ Letter Writer** agent "
                "using the sidebar on the left."
            )
        return (
            "Saya adalah **Penjana Laporan** — saya hanya boleh menghasilkan laporan program, "
            "minit mesyuarat, dan laporan lawatan. Saya tidak dapat menulis surat rasmi.\n\n"
            "👉 Untuk menulis surat rasmi, sila tukar ke ejen **✍️ Penulis Surat** "
            "menggunakan sidebar di sebelah kiri."
        )
...
def _build_report_html(f: dict, session_id: str | None = None) -> str:
    ...
    if nama_pg:
        ...
        for i, nm in enumerate(nama_pg):
            ...
            entry = f"{i+1}) {nm}"
            if jw:
                entry += f"<br><span style='margin-left:1.2em'>{jw}</span>"
            pg_lines.append(entry)
        pegawai_html = "<br>".join(pg_lines)
    else:
        pegawai_html = ""
...
def build_docx(session_id: str) -> bytes | None:
    ...
    def _build_sig_cell(cell, nama, jawatan, tarikh):
        ...
        def _row(label, value):
            ...
            # Hanging indent: left indent = tab stop so wrapped lines align after ":"
            p.paragraph_format.left_indent = Cm(2.8)
            p.paragraph_format.first_line_indent = Cm(-2.8)
